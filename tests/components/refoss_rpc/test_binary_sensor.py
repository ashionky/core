...
async def test_rpc_binary_sensor(
    ...
    entity_id = f"{BINARY_SENSOR_DOMAIN}.test_switch_overpowering"
    await set_integration(hass)

    assert hass.states.get(entity_id).state == STATE_OFF

    mutate_rpc_device_status(
        monkeypatch, mock_rpc_device, "switch:1", "errors", "overpower"
    )
    ...
    assert entry
    assert entry.unique_id == "123456789ABC-switch:1-overpower"
